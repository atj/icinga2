	...
	const Dictionary::Ptr& resolvedMacros, bool useResolvedMacros)
{
	...
	double queriesWarning = MacroProcessor::ResolveMacros("$icingadb_queries_warning$", resolvers, checkable->GetLastCheckResult(),
	    &missingQueriesWarning, MacroProcessor::EscapeCallback(), resolvedMacros, useResolvedMacros);

	double queriesCritical = MacroProcessor::ResolveMacros("$icingadb_queries_critical$", resolvers, checkable->GetLastCheckResult(),
	    &missingQueriesCritical, MacroProcessor::EscapeCallback(), resolvedMacros, useResolvedMacros);

	double pendingQueriesWarning = MacroProcessor::ResolveMacros("$icingadb_pending_queries_warning$", resolvers, checkable->GetLastCheckResult(),
	    &missingPendingQueriesWarning, MacroProcessor::EscapeCallback(), resolvedMacros, useResolvedMacros);

	double pendingQueriesCritical = MacroProcessor::ResolveMacros("$icingadb_pending_queries_critical$", resolvers, checkable->GetLastCheckResult(),
	...
	cr->SetPerformanceData(new Array({
		...
		{ new PerfdataValue("queries_1min", redis->GetQueryCount(60)) },
		{ new PerfdataValue("queries_5mins", redis->GetQueryCount(5 * 60)) },
		{ new PerfdataValue("queries_15mins", redis->GetQueryCount(15 * 60)) },
